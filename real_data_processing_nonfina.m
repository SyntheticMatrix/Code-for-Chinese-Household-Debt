...
% Compute the target number for the top 20% and bottom 80%
...
% Update the actual indices for the top 20% and bottom 80%
actual_20 = target_20 + front_adjust;
actual_80_start = actual_20 + 1;
actual_80_end = total_families - back_adjust;
% Grouping
group_20 = Ill(1:actual_20); % Top 20% of families
group_80 = Ill(actual_80_start:actual_80_end); % Bottom 80% of families
...
% Assume wealth is already sorted in ascending order and has been divided into top 20% and bottom 80%
% Continuing from the previous code:
% group_20: Wealth vector for the top 20% of families
% group_80: Wealth vector for the bottom 80% of families
...
% If divisible, group directly; otherwise, handle the remaining families
extra_20 = num_20 - group_20_size * num_groups;  % Extra families
group_20_avg_ill = zeros(num_groups, 1);  % Store the average wealth for each group
...
for i = 1:num_groups
    ...
    group_20_avg_ill(i) = mean(group_20(start_idx:end_idx));
end
...
group_80_avg_ill = zeros(num_groups, 1);  % Store the average wealth for each group
...
for i = 1:num_groups
    ...
    % Compute the average wealth for each group
    group_80_avg_ill(i) = mean(group_80(start_idx:end_idx));
end

% Output check
fprintf('Size of the average wealth vector for the top 20%% groups: %d x 1\n', length(group_20_avg_ill));
fprintf('Size of the average wealth vector for the bottom 80%% groups: %d x 1\n', length(group_80_avg_ill));

%% 
% Assume the variables income, consumption, and assets are represented as income, consumption, and assets
epsilon = 1e-6; % Avoid zero values in logarithmic transformation
w1_ill_real = log(group_20_avg_ill + 1 + epsilon);
w2_ill_real = log(group_80_avg_ill + 1 + epsilon);
